...
\date{December 14, 2025 (Corrected Version)}
...
\textbf{ERRATUM / CORRECTION NOTICE}\\[0.5em]
This is a \textbf{corrected version} of the original paper. The original results (November 2025) reported compression ratios of 1775$\times$ for news and 187$\times$ for Wikipedia. Independent validation revealed these values were inflated due to use of \textbf{templated sentences} instead of real text.\\[0.5em]
\textbf{Corrected values} (validated with real Wikipedia/CC-News corpora):\\
$\bullet$ News: 1775$\times$ $\rightarrow$ \textbf{178$\times$} (90\% reduction, 13.4\% loss)\\
$\bullet$ Wikipedia: 187$\times$ $\rightarrow$ \textbf{167$\times$} (11\% reduction, 16.2\% loss)\\[0.5em]
The original version remains archived at Zenodo with DOI for transparency. See \texttt{CRITICAL\_FINDINGS.md} in the validation repository for full analysis.
}}
\end{center}
...
\begin{abstract}
High-dimensional embedding vectors (typically 768D for BERT-base) pose significant storage and transmission challenges in modern machine learning systems. While conventional compression techniques achieve modest ratios (1.1-10x), we demonstrate that embeddings with low-dimensional manifold structure enable significant compression. Through rigorous analysis of correlation dimension $D_2$ and Lyapunov exponents $\lambda_1$, we identify datasets where embeddings inhabit low-dimensional manifolds within the nominal high-dimensional space. We validate our approach on \textbf{real BERT-base-uncased embeddings} from Wikipedia and CC-News datasets, achieving \textbf{178$\times$ compression with 13.4\% loss} (measured by cosine similarity) on news articles and \textbf{167$\times$ on Wikipedia with 16.2\% loss}. \textit{Note: Initial results reporting 1775$\times$ were based on templated sentences; these corrected values reflect validation with real Wikipedia/CC-News text.} Our PCA-based attractor compression demonstrates the viability of exploiting low-dimensional manifold structure in embedding spaces. Our theoretical framework connects dynamical systems theory with information theory, providing compression insights based on $D_2$. We provide complete experimental validation, root cause analysis of why standard methods fail, and open-source implementation.
\end{abstract}
...
\subsection{Summary of Contributions (CORRECTED)}

\begin{enumerate}
\item \textbf{Real-world validation on BERT embeddings}: Achieved \textbf{178$\times$ compression on news articles (13.4\% loss)} and \textbf{167$\times$ on Wikipedia sentences (16.2\% loss)} using real BERT-base-uncased (768D) embeddings from actual Wikipedia and CC-News corpora

\item \textbf{Critical data source correction}: Initial results claiming 1775$\times$ were based on templated sentences; validation with real text reduced this to 178$\times$ (90\% reduction) but still demonstrates significant compression
...
\item \textbf{Novel algorithm}: Attractor-based compression via PCA+delta achieves 167-178$\times$ on real BERT data, demonstrating viability of manifold-based compression
...
\item \textbf{Transparent correction}: This version corrects overestimated results from the original publication, demonstrating commitment to scientific integrity
